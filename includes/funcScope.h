...
class FuncScope {
public:
  FuncScope() : funcName(), parentFuncScope(nullptr), table(new SymbolTable()), funcs(){}
  FuncScope(const std::string& name) : funcName(name), parentFuncScope(nullptr), table(new SymbolTable()), funcs(){}
  FuncScope(const std::string& name, FuncScope* funcS) : funcName(name), parentFuncScope(funcS), table(new SymbolTable()), funcs(){}
  FuncScope(FuncScope*);
  ~FuncScope();
